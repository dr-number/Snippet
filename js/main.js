...
const INDEX_DATE = 1;
const INDEX_TIME = 2;
const INDEX_IS_ONLINE = 3;

sendArray[INDEX_IS_ONLINE] = "online"
...
$(document).ready(function(){
  ...
  for(let i = 0; i < 6; ++i) {
    ...
    $("#book-times").append(
      ' <div class="book-time" data-date="'+ moment(date).format('YYYY-MM-DD') +'">\n' +
  ...
  $(".book-day").on('click', function(){
   ...
   if(time) {
     time = time.replace(new RegExp(":00", "g"), "")
     time = time.replace(new RegExp("0", "g"), "")
   }
   else
     time = "00-00";
  ...
  $(".book-switch-btn").on('click', function(){
    ...
    sendArray[INDEX_IS_ONLINE] = isOnline;
